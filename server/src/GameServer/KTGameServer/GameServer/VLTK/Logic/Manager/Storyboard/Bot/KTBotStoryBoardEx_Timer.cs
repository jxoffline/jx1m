...
namespace GameServer.KiemThe.Logic
{
    ...
    public partial class KTBotStoryBoardEx
    {
        ...
        #region Define
        /// <summary>
        /// Định nghĩa StoryBoard của các đối tượng khác
        /// </summary>
        private class OtherStoryBoard
        {
            /// <summary>
            /// Chủ nhân StoryBoard
            /// </summary>
            ...
            public bool IsTickTime
            {
                get
                {
                    return KTGlobal.GetCurrentTimeMilis() - this.LastTickTimer >= KTBotStoryBoardEx.PeriodTick;
                }
            }
        }
        ...
        /// </summary>
        private readonly ConcurrentDictionary<int, OtherStoryBoard> objectTimers = new ConcurrentDictionary<int, OtherStoryBoard>();
        #endregion
        ...
        public void StartTimer()
        {
            ...
            timer.Elapsed += (o, e) => {
                ...
                {
                    LogManager.WriteLog(LogTypes.TimerReport, string.Format("Terminate => {0}", "KTOtherObjectStoryBoard"));
                    /// Ngừng luồng
                    timer.Stop();
                    return;
                }
                ...
                for (int i = 0; i < this.worker.Length; i++)
                {
                    ...
                    {
                        LogManager.WriteLog(LogTypes.TimerReport, string.Format("Tick alive => KTOtherObjectStoryBoard[{0}]", i));
                    ...
                    if (this.worker[i].IsBusy)
                    {
                        ...
                        {
                            LogManager.WriteLog(LogTypes.TimerReport, string.Format("Timeout => KTOtherObjectStoryBoard[{0}]", i));
        ...
        private void DoBackgroundWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                foreach (int key in keys)
                {
                    ...
                    if (!this.objectTimers.TryGetValue(key, out OtherStoryBoard storyBoard))
                    {
                        /// Bỏ qua
                        continue;
                    }
                    ...
                    if (storyBoard.Owner.RoleID % ServerConfig.Instance.MaxBotTimer != workerID)
                    {
                        continue;
                    }
