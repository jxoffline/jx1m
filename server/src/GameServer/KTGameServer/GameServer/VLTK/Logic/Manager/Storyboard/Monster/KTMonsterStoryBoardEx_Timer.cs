...
{
	/// <summary>
	/// Quản lý Timer StoryBoard của quái
	/// </summary>
	public partial class KTMonsterStoryBoardEx
	{
		...
		/// </summary>
		private class MonsterStoryBoard
		{
			/// <summary>
			/// Chủ nhân
			/// </summary>
			...
			public bool IsTickTime
			{
				get
				{
					return KTGlobal.GetCurrentTimeMilis() - this.LastTickTimer >= KTMonsterStoryBoardEx.PeriodTick;
				}
			}
		}
		...
		/// </summary>
		private readonly ConcurrentDictionary<int, MonsterStoryBoard> objectTimers = new ConcurrentDictionary<int, MonsterStoryBoard>();
		#endregion
		...
		public void StartTimer()
		{
			...
			timer.Elapsed += (o, e) => {
				...
				{
					LogManager.WriteLog(LogTypes.TimerReport, string.Format("Terminate => {0}", "KTMonsterStoryBoard"));
					/// Ngừng luồng
					timer.Stop();
					return;
				}
				...
				for (int i = 0; i < this.worker.Length; i++)
				{
					...
					{
						LogManager.WriteLog(LogTypes.TimerReport, string.Format("Tick alive => KTMonsterStoryBoard[{0}]", i));
					...
					if (this.worker[i].IsBusy)
					{
						...
						{
							LogManager.WriteLog(LogTypes.TimerReport, string.Format("Timeout => KTMonsterStoryBoard[{0}]", i));
		...
		private void DoBackgroundWork(object sender, DoWorkEventArgs e)
		{
			try
			{
				...
				foreach (int key in keys)
				{
					...
					if (!this.objectTimers.TryGetValue(key, out MonsterStoryBoard storyBoard))
					{
						/// Bỏ qua
						continue;
					}
					...
					if (storyBoard.Owner.RoleID % ServerConfig.Instance.MaxMonsterTimer != workerID)
					{
						continue;
					}
