...
namespace GameServer.KiemThe.Logic
{
    ...
    public partial class KTTraderCarriageStoryBoardEx
    {
        #region Support
        /// <summary>
        /// Kiểm tra có StoryBoard của xe tiêu tương ứng đang thực thi không
        /// </summary>
        /// <param name="obj"></param>
        /// <returns></returns>
        public bool HasStoryBoard(TraderCarriage obj)
        {
        ...
        /// <param name="obj"></param>
        /// <returns></returns>
        public string GetCurrentPathString(TraderCarriage obj)
        {
            try
            {
                if (this.objectTimers.TryGetValue(obj.RoleID, out TraderCarriageStoryBoard storyBoard))
                {
        ...
        public void ChangeAction(TraderCarriage obj, KE_NPC_DOING action)
        {
            try
            {
        ...
        public void Remove(TraderCarriage obj, bool synsClient = true)
        {
            /// Toác
            if (obj == null)
            {
            ...
            try
            {
                ...
                if (this.objectTimers.TryGetValue(obj.RoleID, out TraderCarriageStoryBoard objectTimer))
                {
                    /// Đánh dấu đã bị hủy
                    ...
                    if (synsClient)
                    {
                        this.SendTraderCarriageStopMoveToClient(obj);
                    }
                }
            }
            catch (Exception ex)
        ...
        public void Add(TraderCarriage carriage, Point fromPos, Point toPos, KE_NPC_DOING action = KE_NPC_DOING.do_run, bool forceMoveBySpecialState = false, bool useAStarPathFinder = false)
        {
            try
            {
                if (carriage == null)
                ...
                this.SendTraderCarriageMoveToClient(carriage, pathStr, fromPos, toPos, action);

                //Console.WriteLine("TraderCarriage {3} => Move from {0} to {1} with paths = {2}", fromPos.ToString(), toPos.ToString(), pathStr, carriage.RoleName);
                ...
                TraderCarriageStoryBoard storyBoard = new TraderCarriageStoryBoard()
                {
                    Owner = carriage,
                    Paths = paths,
        ...
        /// <param name="action"></param>
        public void Add(TraderCarriage obj, string pathString, KE_NPC_DOING action = KE_NPC_DOING.do_run, bool forceMoveBySpecialState = false)
        {
            try
            {
                if (obj == null)
