...
namespace GameServer.KiemThe.Core.Activity.RechageEvent
{
    public class RechageManager
    {
        ...
        {
            string Files = KTGlobal.GetDataPath(KT_FistRechage_XML);
            ...
            Files = KTGlobal.GetDataPath(KTDayRechage_XML);
            ...
            Files = KTGlobal.GetDataPath(TotalRechage_XML);
            ...
            Files = KTGlobal.GetDataPath(KTTotalConsume_XML);
        ...
        public static string GetFistRechage(KPlayer client)
        {
            ...
            totalChongZhiMoney = KT_TCPHandler.QueryTotalRecharge(client);
        ...
        public static string GetDayRechage(KPlayer client, TCPClientPool tcpClientPool, TCPOutPacketPool pool)
        {
            ...
            int totalChongZhiMoneyToday = KT_TCPHandler.QueryTotalRechargeToday(client);
        ...
        private static TCPProcessCmdResults GetFirstChargeAward(TMSKSocket socket, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                {
                    if (!ItemManager.CreateItem(Global._TCPManager.TcpOutPacketPool, client, _ItemGet.ItemID, _ItemGet.Number, 0, "FISTRECHAGE", false, 1, false, ItemManager.ConstGoodsEndTime))
                    {
                        KTPlayerManager.ShowNotification(client, "Có lỗi khi nhận vật phẩm");
                    }
                }
                    ...
                    case ActivityTypes.MeiRiChongZhiHaoLi:
                        {
                            ...
                            foreach (string item in TotalItem)
                            {
                                ...
                                if (!ItemManager.CreateItem(Global._TCPManager.TcpOutPacketPool, client, ItemID, Number, 0, "DAYRECHAGE", false, 1, false, ItemManager.ConstGoodsEndTime, "", -1, "", 0, 1, true))
                                {
                                    KTPlayerManager.ShowNotification(client, "Có lỗi khi nhận vật phẩm");
                                }
                            }
                    ...
                    case ActivityTypes.TotalCharge:
                        {
                            ...
                            if (_Find == null)
                            {
                                strcmd = string.Format("{0}:{1}:{2}", (int)ActivityErrorType.ACTIVITY_NOTEXIST, nActivityType,1);
                                tcpOutPacket = TCPOutPacket.MakeTCPOutPacket(pool, strcmd, nID);
                                return TCPProcessCmdResults.RESULT_DATA;
                            }
                            ...
                            int totalMoney = KT_TCPHandler.QueryTotalRecharge(client);
                            ...
                            {
                                strcmd = string.Format("{0}:{1}:{2}", (int)ActivityErrorType.NOTCONDITION, nActivityType,1);
                                tcpOutPacket = TCPOutPacket.MakeTCPOutPacket(pool, strcmd, nID);
                                return TCPProcessCmdResults.RESULT_DATA;
                            }
                            ...
                            {
                                // Thông báo đã mút rồi
                                strcmd = string.Format("{0}:{1}:{2}", (int)ActivityErrorType.ALREADY_GETED, nActivityType, 1);
                                tcpOutPacket = TCPOutPacket.MakeTCPOutPacket(pool, strcmd, nID);
                                return TCPProcessCmdResults.RESULT_DATA;
                            }
                            ...
                            foreach (string item in TotalItem)
                            {
                                ...
                                if (!ItemManager.CreateItem(Global._TCPManager.TcpOutPacketPool, client, ItemID, Number, 0, "TOTALRECHAGE", false, 0, false, ItemManager.ConstGoodsEndTime, "", -1, "", 0, 1, true))
                                {
                                    KTPlayerManager.ShowNotification(client, "Có lỗi khi nhận vật phẩm");
                                }
                            }
                    ...
                    case ActivityTypes.TotalConsume:
                        {
                            ...
                            foreach (string item in TotalItem)
                            {
                                ...
                                if (!ItemManager.CreateItem(Global._TCPManager.TcpOutPacketPool, client, ItemID, Number, 0, "TOTALCONSUME", false, 0, false, ItemManager.ConstGoodsEndTime, "", -1, "", 0, 1, true))
                                {
                                    KTPlayerManager.ShowNotification(client, "Có lỗi khi nhận vật phẩm");
                                }
                            }
