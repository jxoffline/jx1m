...
{
    /// <summary>
    /// Script Bạch Hổ Đường tầng 3
    /// </summary>
    public class BaiHuTang_Script_Round3 : GameMapEvent
    {
        #region Constants
        /// <summary>
        ...
        public BaiHuTang_Script_Round3(GameMap gameMap, KTActivity activity) : base(gameMap, activity)
        {
        ...
        protected override void OnTick()
        {
            ...
            if (KTGlobal.GetCurrentTimeMilis() - this.LastChangePKModeTick >= BaiHuTang_Script_Round3.ChangePlayersPKModeEveryTick)
            {
            ...
            if (this.nStep == 0)
            {
                ...
                if (this.LifeTimeTicks >= BaiHuTang.Round3.Boss.SpawnAfter)
                {
                    /// Chuyển Step
                    this.nStep = 100;
                    /// Tạo Boss
                    ...
                    this.UpdateEventDetailsToPlayers("Bạch Hổ Đường - Tầng 3", this.DurationTicks - this.LifeTimeTicks, string.Format("Đánh bại <color=yellow>{0}</color>.", "Hộ Đồ Sứ"));
                }
                else
                {
                    /// Nếu đã đến thời gian thông báo thông tin sự kiện
                    if (KTGlobal.GetCurrentTimeMilis() - this.LastNotifyTick >= BaiHuTang_Script_Round3.NotifyActivityInfoToPlayersEveryTick)
                    {
                        /// Đánh dấu thời gian thông báo thông tin sự kiện
                        this.LastNotifyTick = KTGlobal.GetCurrentTimeMilis();
                        /// Cập nhật thông tin sự kiện
                        this.UpdateEventDetailsToPlayers("Bạch Hổ Đường - Tầng 3", BaiHuTang.Round3.Boss.SpawnAfter - this.LifeTimeTicks, string.Format("Đợi <color=yellow>{0}</color> xuất hiện.", "Hộ Đồ Sứ"));
                    }
                }
            }
            /// Nếu ở Step 1
            else if (this.nStep == 1)
            {
                ...
                /// Thông báo mở tầng
                this.NotifyAllPlayers("Hoàn tất Bạch Hổ Đường, có thể trở lại Đại Điện rồi!");
                ...
                this.LastNotifyTick = KTGlobal.GetCurrentTimeMilis();
                /// Cập nhật thông tin sự kiện
                this.UpdateEventDetailsToPlayers("Bạch Hổ Đường - Tầng 3", this.DurationTicks - this.LifeTimeTicks, string.Format("Hoàn tất <color=green>{0}</color>", "Bạch Hổ Đường"));
            }
        }
            ...
            if (obj == this.boss)
            {
                ...
                foreach (KPlayer otherPlayer in this.GetPlayers())
                {
                    ...
                    if ((otherPlayer.GuildID >= 0 && otherPlayer.GuildID == player.GuildID) || (otherPlayer.FamilyID >= 0 && otherPlayer.FamilyID == player.FamilyID))
                    {
                        ...
                        prestige = BaiHuTang.Round3.Repute.GuildKillBossPrestige;
                        /// Tăng điểm danh vọng theo bang hội giết Boss
                        repute = BaiHuTang.Round3.Repute.GuildKillBossRepute;
                    }
                    else
                    {
                        /// Tăng điểm uy danh qua ải thường
                        prestige = BaiHuTang.Round3.Repute.OtherPrestige;
                        /// Tăng điểm danh vọng qua ải thường
                        repute = BaiHuTang.Round3.Repute.OtherRepute;
                    }
        ...
        public override void OnPlayerEnter(KPlayer player)
        {
            ...
            /// Cập nhật thông tin
            this.UpdateEventDetailsToPlayers("Bạch Hổ Đường - Tầng 3", BaiHuTang.Round3.Boss.SpawnAfter, string.Format("Đợi <color=yellow>{0}</color> xuất hiện.", "Hộ Đồ Sứ"));
        }
		...
		public override bool OnPlayerClickReliveButton(KPlayer player)
        {
            ...
            KTPlayerManager.Relive(player, BaiHuTang.Round3.OutMaps[this.Map.MapCode].OutMapID, BaiHuTang.Round3.OutMaps[this.Map.MapCode].OutPosX, BaiHuTang.Round3.OutMaps[this.Map.MapCode].OutPosY, 100, 100, 100);
            return true;
        }
        #endregion
        ...
        private void CreateMonsters()
        {
            ...
            foreach (BaiHuTang.RoundInfo.MonsterInfo monsterInfo in BaiHuTang.Round3.Monsters)
            {
                /// Ngũ hành
        ...
        {
            /// Vị trí xuất hiện
            KeyValuePair<int, int> randomPos = BaiHuTang.Round3.Boss.RandomPos[KTGlobal.GetRandomNumber(0, BaiHuTang.Round3.Boss.RandomPos.Count - 1)];
            /// Ngũ hành
            KE_SERIES_TYPE series = KE_SERIES_TYPE.series_none;
            /// Hướng quay
            KiemThe.Entities.Direction dir = KiemThe.Entities.Direction.NONE;
            /// Mức máu
            int hp = BaiHuTang.Round3.Boss.BaseHP + BaiHuTang.Round3.Boss.HPIncreaseEachLevel * this.Level;

            /// Tạo Boss
            Monster monster = KTMonsterManager.Create(new KTMonsterManager.DynamicMonsterBuilder()
            {
                MapCode = this.Map.MapCode,
                ResID = BaiHuTang.Round3.Boss.IDByActivity[this.Activity.Data.ID],
                PosX = randomPos.Key,
                PosY = randomPos.Value,
                Name = BaiHuTang.Round3.Boss.Name,
                Title = BaiHuTang.Round3.Boss.Title,
                MaxHP = hp,
                Level = this.Level,
                MonsterType = (MonsterAIType) BaiHuTang.Round3.Boss.AIType,
                AIID = BaiHuTang.Round3.Boss.AIScriptID,
                Camp = 65535,
            });
        ...
        {
            KDynamicArea teleport = KTDynamicAreaManager.Add(this.Map.MapCode, -1, BaiHuTang.Round3.Teleport.Name, BaiHuTang.Round3.Teleport.ResID, BaiHuTang.Round3.Teleport.PosX, BaiHuTang.Round3.Teleport.PosY, -1, 2000, BaiHuTang.Round3.Teleport.Radius, -1, null);
            teleport.OnEnter = (obj) => {
                if (obj is KPlayer)
                {
                    this.KickOutPlayer(obj as KPlayer);
                }
            };
        }
        ...
        {
            KTPlayerManager.ChangeMap(player, BaiHuTang.Round3.OutMaps[this.Map.MapCode].OutMapID, BaiHuTang.Round3.OutMaps[this.Map.MapCode].OutPosX, BaiHuTang.Round3.OutMaps[this.Map.MapCode].OutPosY);
        }
