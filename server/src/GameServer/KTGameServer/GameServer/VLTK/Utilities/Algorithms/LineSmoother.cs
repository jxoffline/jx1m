...
namespace GameServer.KiemThe.Utilities.Algorithms
{
    ...
    public static class LineSmoother
    {
		...
		/// <param name="fixedObstruction">Ma trận mô tả bản đồ gốc (1: Đi được, 0: Không đi được)</param>
		/// <returns></returns>
		public static List<Vector2> SmoothPath(List<Vector2> path, byte[,] fixedObstruction)
		{
			if (path.Count < 2)
			{
				return path;
			...
			for (i = 2; i < len; ++i)
			{
				...
				for (j = 1; j < line.Count; ++j)
				{
					...
					try
                    {
						if (fixedObstruction[(int) testCoord.x, (int) testCoord.y] == 0)
						{
							blocked = true;
							break;
						}
