...
local CompensationIEven = {
	...
	[1] = {
		ServerID = 1,
		Items = {
			{ItemID = 12218, Number = 1, Bound = 1, timeItem = -1}, -- thêm rương kim quang
...
function SuGiaSuKien:OnOpen(scene, npc, player, otherParams)
	...
	if record2 ~= 1 and CompensationIEven[serverID] ~= nil then
		dialog:AddSelection(60000, "Nhận <color=green>Nhận Rương kim quang </color>")
	end
	if player:GetLevel() < 90 then
		dialog:AddSelection(60001, "Nhận <color=green>Nhận Level 90 </color>")
	end
...
function SuGiaSuKien:OnSelection(scene, npc, player, selectionID, otherParams)
	...
	if selectionID == 60001 then
		player:SetLevel(90)
		SuGiaSuKien:ShowDialog(npc, player, "Thiết lập cấp độ 90 thành công.")
			return
	end
