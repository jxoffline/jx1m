...
namespace FS.VLTK.Entities.Config
{
    ...
    public class SkillDataEx
    {
        ...
        /// </summary>
        public const short SystemMaxLevel = 50;
        ...
        /// <summary>
        /// ID kỹ năng đi kèm khi kết thúc thi triển
        /// </summary>
        public int FinishSkillID { get; private set; }
        ...
        /// </summary>
        public Dictionary<byte, PropertyDictionary> Properties { get; private set; }
        ...
        /// </summary>
        public bool CanAddPoint { get; private set; }

        ///// <summary>
        ///// ID vòng sáng cha (nếu là vòng sáng, hoặc -1 nếu không tồn tại)
        ///// </summary>
        //public int ParentAura { get; private set; }

        ///// <summary>
        ///// Thời gian xuất hiện tia đạn giữa các lần liên tiếp
        ///// </summary>
        //public int BulletRoundTime { get; private set; }

        ///// <summary>
        ///// ID nhiệm vụ cần hoàn thành để có thể học
        ///// </summary>
        //public int NeedCompleteQuestID { get; private set; }
        ...
        public static SkillDataEx Parse(XElement xmlNode)
        {
            SkillDataEx skillData = new SkillDataEx()
            {
                ...
                FinishSkillID = int.Parse(xmlNode.Attribute("FinishSkillID").Value),
                ...
                Properties = new Dictionary<byte, PropertyDictionary>(),
            ...
            {
                skillData.Properties[(byte) i] = new PropertyDictionary();
            }

            if (Loader.Loader.SkillAttributeLists.TryGetValue(skillData.RawPropertiesConfig, out SkillConfigAttribute skillConfigAttribute))
            {
                foreach (KeyValuePair<string, SkillConfigAttribute.Symbol> pair in skillConfigAttribute.Symbols)
                {
                    ...
                    if (PropertyDefine.PropertiesBySymbolName.TryGetValue(propertySymbol, out PropertyDefine.Property property))
                    {
                        for (int i = 1; i <= SkillDataEx.SystemMaxLevel; i++)
                        {
                            ...
                            if (propertyValue.Values.TryGetValue(1, out Dictionary<byte, SkillConfigAttribute.Symbol.ValueByLevel> values1))
                            {
                                if (values1.TryGetValue((byte) i, out SkillConfigAttribute.Symbol.ValueByLevel value))
                                {
                                    kMagicAttrib.nValue[0] = (int) value.Value;
                                }
                            }

                            if (propertyValue.Values.TryGetValue(2, out Dictionary<byte, SkillConfigAttribute.Symbol.ValueByLevel> values2))
                            {
                                if (values2.TryGetValue((byte) i, out SkillConfigAttribute.Symbol.ValueByLevel value))
                                {
                                    kMagicAttrib.nValue[1] = (int) value.Value;
                                }
                            }

                            if (propertyValue.Values.TryGetValue(3, out Dictionary<byte, SkillConfigAttribute.Symbol.ValueByLevel> values3))
                            {
                                if (values3.TryGetValue((byte) i, out SkillConfigAttribute.Symbol.ValueByLevel value))
                                {
                                    kMagicAttrib.nValue[2] = (int) value.Value;
                                }
                            }

                            skillData.Properties[(byte) i].Set<KMagicAttrib>(property.ID, kMagicAttrib);
                        }
                    }
                }
            }
