...
namespace FS.VLTK.UI.Main.Welfare.Recharge
{
    ...
    /// Khung tích tiêu
    /// </summary>
    public class UIWelfare_Recharge_TotalConsume : MonoBehaviour
    {
        #region Define
        /// <summary>
        /// Button đóng khung
        ...
        /// Sự kiện nhận quà
        /// </summary>
        public Action<ConsumeAward> Get { get; set; }

        private TotalConsume _Data;
        /// <summary>
        /// Dữ liệu
        /// </summary>
        public TotalConsume Data
        {
            get
            {
                return this._Data;
        ...
        private void AddItemList(int pos, ConsumeAward awardsInfo)
        {
        ...
        private void Refresh()
        {
            ...
            /// Xóa Text tổng số đã tiêu
            this.UIText_TotalConsume.text = "0";
            ...
            /// Nếu không thỏa mãn kích thước
            if (infoParams.Length != this._Data.ConsumeAward.Count)
            {
                KTGlobal.AddNotification("Dữ liệu truyền về có lỗi, hãy liên hệ với hỗ trợ để được trợ giúp!");
                return;
            }
            ...
            try
            {
                ...
                /// Cập nhật tổng số đã tiêu
                this.UIText_TotalConsume.text = KTGlobal.GetDisplayMoney(int.Parse(infoParamsS1[1]));
            }
            catch (Exception)
            {
                KTGlobal.AddNotification("Dữ liệu truyền về có lỗi, hãy liên hệ với hỗ trợ để được trợ giúp!");
            ...
            for (int i = 0; i < this._Data.ConsumeAward.Count; i++)
            {
                /// Thêm danh sách vật phẩm thưởng
                this.AddItemList(i, this._Data.ConsumeAward[i]);
            }
