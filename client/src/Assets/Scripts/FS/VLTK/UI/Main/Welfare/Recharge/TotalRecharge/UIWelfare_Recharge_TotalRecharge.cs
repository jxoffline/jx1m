...
namespace FS.VLTK.UI.Main.Welfare.Recharge
{
    ...
    /// Khung tích nạp
    /// </summary>
    public class UIWelfare_Recharge_TotalRecharge : MonoBehaviour
    {
        #region Define
        /// <summary>
        /// Button đóng khung
        ...
        /// </summary>
        [SerializeField]
        private TextMeshProUGUI UIText_TotalRecharged;
        #endregion
        ...
        /// Sự kiện nhận quà
        /// </summary>
        public Action<TotalRechageAward> Get { get; set; }

        private TotalRechage _Data;
        /// <summary>
        /// Dữ liệu
        /// </summary>
        public TotalRechage Data
        {
            get
            {
                return this._Data;
        ...
        private void AddItemList(int pos, TotalRechageAward awardsInfo)
        {
        ...
        private void Refresh()
        {
            ...
            /// Xóa Text tổng số đã nạp
            this.UIText_TotalRecharged.text = "0";
            ...
            /// Nếu không thỏa mãn kích thước
            if (infoParams.Length != this._Data.TotalRechageAward.Count)
            {
                KTGlobal.AddNotification("Dữ liệu truyền về có lỗi, hãy liên hệ với hỗ trợ để được trợ giúp!");
                return;
            }
            ...
            try
            {
                ...
                /// Cập nhật tổng số đã nạp
                this.UIText_TotalRecharged.text = KTGlobal.GetDisplayMoney(int.Parse(infoParamsS1[1]));
            }
            catch (Exception)
            {
                KTGlobal.AddNotification("Dữ liệu truyền về có lỗi, hãy liên hệ với hỗ trợ để được trợ giúp!");
            ...
            for (int i = 0; i < this._Data.TotalRechageAward.Count; i++)
            {
                /// Thêm danh sách vật phẩm thưởng
                this.AddItemList(i, this._Data.TotalRechageAward[i]);
            }
